...
public sealed class RoundTripTests : DesugarTestBase
{
    ...
    [InlineData("def c = true;\nloop { if c { break; } print(1); break; }\nprint(2);")]
    [InlineData("def x = 1;\ndef y = loop { break 2; };\nprint(x);")]
    [InlineData("loop :a { loop :b { loop :c { break :c; } } }")]
    [InlineData("loop :a { continue :a; }")]
    [InlineData("loop :a { loop :b { break :a; } }")]
    [InlineData("def f = fn() Void { loop { break; } print(1); };")]
    [InlineData("def x = { loop { break; } 1 };")]
    [InlineData("var x = 1;")]
