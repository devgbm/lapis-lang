...
Arrays passam a carregar o tamanho no tipo (`[Int;3]`), com construção
`.[1, 2, 3]` — o mesmo ponto de `.User { }`. **Q29** (`[T;N]` atribuível a
`[T;?]`) bloqueia o plano 24; **Q30** (aritmética de tamanho) fica adiada.

| # | Plano | Projeto | Milestone |
|---|---|---|---|
| 24 | [Arrays com tamanho no tipo](24-sized-arrays.md) | `Lapis.Ast` + `Lapis.TypeChecker` + `prelude.ls` | M12 ⏳ Q29 |
...
| **M12** | Arrays com tamanho | `[Int;3]`, `.[1,2,3]`, índice literal checado, `length` constante — **bloqueado por Q29** | 24 |
...
**Por que arrays vêm primeiro.** O plano 24 muda a sintaxe de tipo de array e o
`prelude.ls` junto — `TypeInfo` tem quatro campos de array. Migrar prelude,
exemplos e corpus é a maior parte daquele plano, e fazê-la **antes** dos type
members evita migrar duas vezes.
